...
impl Applet for Sha512sumApplet {
    ...
    fn run(&self, args: &[String]) -> Result<i32, Box<dyn std::error::Error>> {
        run_hash_applet::<Sha512Hasher>("sha512sum", args)
    }
}
...
impl HashImpl for Sha512Hasher {
    fn new() -> Self {
        Sha512Hasher {
            state: [
                0x6a09e667f3bcc908,
    ...
    fn finalize(mut self) -> String {
        ...
        let buffer_len = (self.len % 128) as usize;
        if buffer_len > 112 {
            self.update(&[0; 128][..128 - buffer_len]);
        }
        let padding = 112 - (self.len % 128) as usize;
        self.update(&[0; 128][..padding]);
